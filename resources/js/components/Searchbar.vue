<template>
  <div>
    <n-config-provider :theme="theme">
      <n-card style="max-width: 1200px; margin: 20px auto;" :bordered="false">
            ...
            <div class="guests-dropdown" v-show="showGuestsDropdown" @click.stop>
              ...
              <div class="guest-row">
                <div class="guest-info">
                  ...
                  <div class="guest-description">Od 13 godina naviše</div>
                </div>
              ...
              <div class="guest-row">
                <div class="guest-info">
                  ...
                  <div class="guest-description">Od 2 do 12 godina</div>
                </div>
                <div class="counter-controls">
                  <n-button
                    circle
              ...
              <div class="guest-row">
                <div class="guest-info">
                  ...
                  <div class="guest-description">Do 2 godine</div>
                </div>
                <div class="counter-controls">
                  <n-button
                    circle
        ...
        <div v-if="searchResult" style="margin-top: 20px;">
          <n-alert type="info" title="Parametri pretrage:">
            <ul style="margin: 0; padding-left: 20px;">
              <li><strong>Destinacija:</strong> {{ getDestinationLabel(searchResult.destination) }}</li>
              <li><strong>Dolazak:</strong> {{ formatDate(searchResult.checkIn) }}</li>
              <li><strong>Odlazak:</strong> {{ formatDate(searchResult.checkOut) }}</li>
              <li><strong>Odrasli:</strong> {{ searchResult.adults }}</li>
              <li v-if="searchResult.children > 0"><strong>Deca:</strong> {{ searchResult.children }}</li>
              <li v-if="searchResult.infants > 0"><strong>Bebe:</strong> {{ searchResult.infants }}</li>
              <li><strong>Broj noći:</strong> {{ calculateNights(searchResult.checkIn, searchResult.checkOut) }}</li>
            </ul>
          </n-alert>
        </div>
      </n-card>
...
const guestsDisplayText = computed(() => {
  ...
  let text = `${totalGuests} ${totalGuests === 1 ? 'gost' : totalGuests < 5 ? 'gosta' : 'gostiju'}`

  if (searchForm.value.infants > 0) {
    text += `, ${searchForm.value.infants} ${searchForm.value.infants === 1 ? 'beba' : 'beba'}`
  }
...
const isFormValid = computed(() => {
  return searchForm.value.destination &&
...
const getDestinationLabel = (value) => {
  return destinationOptions.find(option => option.value === value)?.label || value
}
...
const handleSearch = () => {
  ...
  if (searchForm.value.checkOut <= searchForm.value.checkIn) {
    validationMessage.value = 'Datum odlaska mora biti nakon datuma dolaska'
    return
  }
