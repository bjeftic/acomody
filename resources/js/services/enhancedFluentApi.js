...
class FluentApiBuilder {
  ...
  // Path segment
  _addPath(segment) {
    const builder = new FluentApiBuilder(this.baseClient,
  ...
  // HTTP Methods
  async get() {
    return await this.baseClient.request('GET', this.path, null, this.queryParams, {
      ...this.requestConfig,
  ...
  async post(data = null) {
    return await this.baseClient.request('POST', this.path, data, this.queryParams, {
      ...this.requestConfig,
      _requiresAuth: this._requiresAuth
    })
  }
  ...
  async put(data = null) {
    return await this.baseClient.request('PUT', this.path, data, this.queryParams, {
      ...this.requestConfig,
      _requiresAuth: this._requiresAuth
    })
  }
  ...
  async patch(data = null) {
    return await this.baseClient.request('PATCH', this.path, data, this.queryParams, {
      ...this.requestConfig,
      _requiresAuth: this._requiresAuth
    })
  }
  ...
  async delete() {
    return await this.baseClient.request('DELETE', this.path, null, this.queryParams, {
      ...this.requestConfig,
      _requiresAuth: this._requiresAuth
    })
  }
...
class FluentApiClient {
  constructor(baseURL = window.location.origin, options = {}) {
    ...
    this.options = {
      publicEndpoints: new Set(['login', 'register', 'forgot-password', 'reset-password']),
  ...
  async request(method, path, data = null, params = {}, config = {}) {
    ...
    const isValid = this._validateEndpoint ? this._validateEndpoint(method, path) : true
  ...
  async login(credentials) {
    ...
    // Then attempt login
    return await this.request('POST', 'login', credentials, {}, { _requiresAuth: false })
  }
  ...
  async register(userData) {
    await this.ensureCsrfCookie()
    return await this.request('POST', 'register', userData, {}, { _requiresAuth: false })
  }
...
// Enhanced FluentApiBuilder with number support and camelCase conversion
  ...
  constructor(baseClient, path = '') {
    ...
    return new Proxy(this, {
      get(target, prop, receiver) {
        ...
        // If it's a regular property (method)
        if (prop in target) {
          return target[prop]
        }
...
class EnhancedFluentApiClient extends FluentApiClient {
  ...
  async loadSwaggerDocs() {
    try {
      ...
      console.log(`Loaded ${this.availableEndpoints.size} API endpoints from Swagger`)

      if (process.env.NODE_ENV === 'development') {
        console.log('Available endpoints:', Array.from(this.availableEndpoints).sort())
      }

    } catch (error) {
      console.warn('Could not load Swagger documentation:', error.message)
      console.log('Endpoint validation disabled - all requests will be allowed')
    }
  }
...
export { FluentApiClient, EnhancedFluentApiClient, FluentApiBuilder, EnhancedFluentApiBuilder }