...
const apiClient = new EnhancedFluentApiClient(window.location.origin, {
  publicEndpoints: new Set([
    'login', 'logIn', 'signUp', 'sign-up', 'register',
    'forgot-password', 'reset-password', 'verify-email',
    'auth', 'public', 'health', 'csrf-cookie'
  ]),
  ...
  onUnauthorized: (error) => {
    ...
    if (typeof window !== 'undefined' && window.Vue) {
      window.Vue.prototype.$bus && window.Vue.prototype.$bus.$emit('auth:unauthorized')
    }
  ...
  onForbidden: (error) => {
    ...
    if (typeof window !== 'undefined' && window.Vue) {
      window.Vue.prototype.$bus && window.Vue.prototype.$bus.$emit('auth:forbidden', {
...
export const createApiCall = (apiCallPromise) => {
  return async (...args) => {
    try {
      ...
      const response = typeof apiCallPromise === 'function' 
      ...
      if (response.success) {
        ...
        const error = new Error(response.error?.message || 'API call failed')
